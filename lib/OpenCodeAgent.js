...
class OpenCodeAgent {
  ...
  async executeWithModel(modelName, prompt, options = {}) {
    ...
    // Use OpenCode for free models, use configured paid-provider for paid models
    if (modelInfo.tier === 'free') {
      return await this.executeWithOpenCode(modelInfo, fullPrompt, options);
    } else {
      return await this.executeWithPaidProvider(modelInfo, fullPrompt, options);
    }
  }
  ...
  // Execute with a paid provider (e.g. opencode/ollama)
  async executeWithPaidProvider(modelInfo, prompt, options) {
    try {
      const response = await this.opencodeClient.session.prompt({
        path: { id: this.sessionId },
        body: {
      ...
      throw new Error(`Paid-provider execution failed: ${error.message}`);
    }
  }
